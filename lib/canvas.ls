...
getCanvas id = do {
    ...
    case x of
      (Just x') -> do {
          n <- getAttr x' "tagName";
          if n == "CANVAS"
            then case _jsfun "(function(x) {return x.getContext('2d');})" 1 x' of
                   ctx | domElemValid ctx -> return (Just (Canvas ctx));
                   _                      -> return Nothing;
            else return Nothing;
        };
...
fillColor (Canvas can) (RGB r g b) =
  case _jsfun "(function (c,r,g,b) {c.fillStyle = 'rgb('+r+','+g+','+b+')'; return 0;})" 4 can r g b of
    0 -> return ();
    ;

-- Fill a rectangle using the current fill color.
fillRect :: Canvas -> Point -> Point -> IO ();
fillRect (Canvas can) (Pt x1 y1) (Pt x2 y2) =
  case _jsfun "(function (c,x1,y1,x2,y2) {c.fillRect(x1,y1,x2,y2); return 0;})" 5 can x1 y1 x2 y2 of
    0 -> return ();
    ;
